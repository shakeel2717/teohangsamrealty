...
this.loadingFunction=function(){};this.doneLoadingFunction=function(){};var loadingTimeout;
this.call=function(sFunction,aArgs,sRequestType)
{	 
	var i,r,postData;if(document.body&&xajaxWaitCursor)
document.body.style.cursor='wait';if(xajaxStatusMessages==true)document.getElementById('status').innerHTML = window.status='Sending Request...';clearTimeout(loadingTimeout);loadingTimeout=setTimeout("xajax.loadingFunction();",400);if(xajaxDebug)this.DebugMessage("Starting xajax...");if(sRequestType==null){var xajaxRequestType=xajaxDefinedPost;}
...
delete r;r=null;}
if(xajaxDebug)this.DebugMessage("Calling "+sFunction+" uri="+uri+" (post:"+postData+")");r.send(postData);if(xajaxStatusMessages==true) document.getElementById('status').innerHTML = window.status='Loading...';delete r;return true;}
...
this.processResponse=function(xml){clearTimeout(loadingTimeout);this.doneLoadingFunction();if(xajaxStatusMessages==true) document.getElementById('status').innerHTML = window.status='Loading...';var tmpXajax=null;xml=xml.documentElement;if(xml==null)
...
delete xml;delete i;document.body.style.cursor='default';if(xajaxStatusMessages==true) {document.getElementById('status').innerHTML = ''; window.status='Done';}}
}
